...
HWP용 JSON fields의 기본 초안 형식은 `--draft-style paragraph`입니다. `WORK_CONTENT`와 호환용 `SUMMARY`/`TASKS`에는 실제 회사 업무를 한 문단(주로 2~3문장)으로 넣으며, Git 활동 상태와 분류 개수 같은 내부 진단은 fields가 아니라 Markdown 검토 정보와 `collection_summary`에만 남깁니다. 과거 섹션형 형식이 꼭 필요할 때만 `--draft-style sectioned`를 명시합니다.
...
```bash
python3 mac_collect_lorotopik_worklog.py \
