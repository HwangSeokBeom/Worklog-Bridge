...
Worklog Bridge는 Mac에서 수행한 작업 기록을 보수적으로 수집·분류해 daily/weekly 근무일지 초안을 만들고, 사람이 검토한 JSON/Markdown을 Windows PC로 옮겨 HWP 템플릿에 입력하는 개인용 로컬 CLI 파이프라인입니다.
...
- 지정한 Git repo에서 commit subject, short SHA, 날짜, 변경 파일 **경로**, shortstat만 수집
- 지정한 작업 메모·계획에서 제한된 제목/bullet/짧은 문장만 추출
- 포함 대상 작업, 제외 대상 개인 작업, 검토 필요 항목으로 보수적 분류
...
기본 HWP 포함 정책은 허용된 작업 기록만 fields에 넣는 방식입니다. 검토 필요 항목은 JSON/Markdown 검토 목록에는 남지만 기본 fields에는 들어가지 않습니다. 제외된 개인 항목은 상세 내용 없이 프로젝트명 수준의 스텁만 남습니다.

## 1. Mac 설정

요구 환경은 Python 3.10+입니다. Mac collector는 외부 Python 패키지가 필요 없습니다.

이 Mac의 저장소 경로에서 안전한 자동 스캐폴드를 먼저 실행할 수 있습니다. 기존 `config.local.json`은 절대 덮어쓰지 않으며, 지정한 프로젝트 계열 이름의 실제 Git repo만 제한 탐색합니다.
...
- `repo_paths`: 수집 대상 repo 절대 경로. 한 개 이상 필요
...
- 포함 대상 힌트: 수집할 repo/작업을 식별할 추가 키워드
...
```bash
"$PYTHON_BIN" worklog_collector.py --config "$CONFIG" --preflight
```
...
```bash
"$PYTHON_BIN" worklog_collector.py --config "$CONFIG" --diagnose
echo $?
```
...
- repo 존재, Git repo 여부, 포함 대상 repo 신호
- 활성화한 notes/plan 경로
...
```bash
"$PYTHON_BIN" worklog_collector.py \
  --config "$CONFIG" \
  --activity-diagnose \
  --mode daily
```
...
dry-run은 날짜 범위, 분류별 개수, Git 활동 상태, privacy 제외 수, 예상 JSON/Markdown 경로와 `files_written: false`를 출력합니다. 범위 내 Git commit이 없으면 `EXPECTED_ZERO_ACTIVITY`와 "선택한 수집 기간에 Git 활동이 발견되지 않았습니다"를 진단 정보로 명시하지만, Git 활동 0건을 업무 0건으로 간주하지 않습니다.

HWP용 JSON fields의 기본 초안 형식은 `--draft-style paragraph`입니다. `WORK_CONTENT`와 호환용 `SUMMARY`/`TASKS`에는 실제 작업 내용을 한 문단(주로 2~3문장)으로 넣으며, Git 활동 상태와 분류 개수 같은 내부 진단은 fields가 아니라 Markdown 검토 정보와 `collection_summary`에만 남깁니다. 과거 섹션형 형식이 꼭 필요할 때만 `--draft-style sectioned`를 명시합니다.

## Hybrid worklog sources

Mac 수집기는 Git 활동 + 수동 업무 기록 + 노트/계획/코멘트를 종합해 일지를 생성합니다. Git 활동은 자동 근거이고, 수동 활동 파일은 fallback이 아니라 매일의 실제 업무를 가장 안정적으로 설명하는 first-class 입력입니다. Git 활동이 있는 날에는 Git 근거와 수동 기록을 함께 반영하고, Git 활동이 없는 날에도 수동 업무 기록이 있으면 정상적인 HWP용 근무 문단을 생성합니다.

지원하는 날짜별 수동 활동 파일 경로는 다음과 같습니다. config에 `manual_activity`를 지정하지 않아도 다음 후보 경로에서 대상 날짜의 `YYYY-MM-DD.md` 또는 `YYYY-MM-DD.txt`를 자동으로 찾습니다.

```text
~/Documents/WorklogBridge/manual/YYYY-MM-DD.md
~/Documents/WorklogBridge/activities/YYYY-MM-DD.md
~/Documents/WorklogBridge/notes/YYYY-MM-DD.md
manual_activity/YYYY-MM-DD.md
worklog_notes/YYYY-MM-DD.md
```

운영 또는 테스트에서 다른 디렉터리를 쓰려면 config의 `manual_activity.paths`를 지정하거나 CLI에 `--manual-activity-dir /path/to/dir`를 추가합니다. 날짜별 수동 활동 파일은 파일명 날짜를 기준으로 수집하므로, 당일 이후에 수정해도 해당 날짜의 업무 기록으로 반영됩니다. 개인 프로젝트 신호가 있는 줄은 기존 privacy/classification 정책에 따라 제외됩니다.

가장 안정적인 운영 방식은 매일 업무 종료 전에 `~/Documents/WorklogBridge/manual/YYYY-MM-DD.md`에 2~5줄 정도로 실제 수행한 업무를 적어두는 것입니다. 노트/계획/코멘트는 보조 맥락으로 함께 반영되고, Git 진단 문구는 Markdown 검토 정보와 `collection_summary`에만 남으며 HWP `WORK_CONTENT`에는 들어가지 않습니다.

실무 명령 예시:

```bash
TODAY="$(date +%F)"
mkdir -p "$HOME/Documents/WorklogBridge/manual"
open -a TextEdit "$HOME/Documents/WorklogBridge/manual/$TODAY.md"
```

특정 날짜의 daily draft 생성(`--date` 옵션은 없으므로 `--since`/`--until` 사용):

```bash
python3 worklog_collector.py \
  --config config.local.json \
  --mode daily \
  --draft-style paragraph \
  --since "2026-07-08T00:00:00" \
  --until "2026-07-08T23:59:59.999999" \
  --manual-activity-dir "$HOME/Documents/WorklogBridge/manual"
```
...
path = Path.home() / "Documents/WorklogBridge/outbox/daily_worklog_2026-07-08.json"
payload = json.loads(path.read_text(encoding="utf-8"))
print(payload["fields"]["WORK_CONTENT"])
PY
```

Gmail이 `config.local.json`에서 `gmail_delivery.enabled=true`로 설정되어 있으면 위 daily 실행이 JSON/Markdown을 먼저 생성한 뒤 Gmail 전송까지 시도합니다. 전송 없이 outbox JSON만 검증하려면 다음처럼 확인합니다.

```bash
python3 -m json.tool "$HOME/Documents/WorklogBridge/outbox/daily_worklog_2026-07-08.json"
```
...
명시적으로 섹션형 초안을 생성하려면 다음 옵션을 추가합니다.

```bash
"$PYTHON_BIN" worklog_collector.py \
  --config "$CONFIG" \
  --mode daily \
  --draft-style sectioned
```
...
설치된 launchd job은 별도 runner나 shell wrapper를 거치지 않습니다. plist의 `ProgramArguments`가 설치 시 확인한 Python 3.10+ binary와 `worklog_collector.py`, `--config <설정 경로> --mode daily`를 각각 독립 argument로 직접 전달하므로 저장소 경로에 공백이 있어도 그대로 동작합니다.
...
```bash
"$PYTHON_BIN" worklog_collector.py --config "$CONFIG" --diagnose
launchctl print "gui/$UID/com.worklogbridge.daily"
launchctl kickstart -k "gui/$UID/com.worklogbridge.daily"
tail -f "$HOME/Documents/WorklogBridge/logs/worklog-daily.log"
tail -f "$HOME/Documents/WorklogBridge/logs/worklog-daily.error.log"
```
...
3. 일반 Gmail password나 다른 계정 password를 사용하지 않습니다. 이 구현은 OAuth를 사용하지 않습니다.
...
실패 시 metadata와 `worklog-daily.error.log`에서 환경 변수 이름, 인증 실패 여부, SMTP 도달 여부를 확인합니다. 실제 Gmail 받은편지함에 제목과 두 첨부 파일이 도착한 것을 확인하기 전에는 production Gmail delivery가 검증된 것이 아닙니다.
...
3. `uncertain`을 포함 대상 작업이라고 자동 가정하지 않습니다. 필요하면 원본 메모를 명확히 고치고 다시 생성합니다.
4. 개인 상세 내용, code/diff, secret-looking 문자열이 없는지 확인합니다.
5. Gmail 수신 message와 delivery-safe 첨부 파일이 의도한 작업 기록만 포함하는지 확인합니다.
...
5. 실제 수집 대상 repo 및 활성화한 notes/plan 경로 확인
...
Windows PC에 Python 3.10+, 한컴오피스 한글을 준비합니다. 허용되는 경우에만 선택 의존성을 설치합니다.
...
기존 템플릿도 다음 legacy 필드 세트를 모두 가지고 있으면 계속 사용할 수 있습니다. `WORK_CONTENT`는 `SUMMARY`와 `TASKS`로 자동 호환됩니다.
...
두 필드 세트 중 하나가 완전하면 필드 검증은 exit 0, 둘 다 불완전하면 3을 반환합니다. `pyhwpx`와 `HWPFrame.HwpObject` 모두 사용할 수 없으면 명확한 오류와 exit 2를 반환합니다. 어떤 output도 쓰지 않습니다.
...
보안 정책이 스크립트/COM 자동화를 막으면 정책을 우회하지 말고 Markdown/TXT를 한글 템플릿에 수동으로 붙여넣습니다.
...
- `collection_summary`: 수집 항목 수, Git 활동 수/상태, 포함된 작업 수, 선택 기간 활동 안내
...
```bash
plutil -lint launchd/com.worklogbridge.daily.plist
...
- **수집 대상 repo로 확인되지 않음:** 잘못된 repo를 넣지 않았는지 확인하고, 실제 수집 대상 repo라면 config의 식별 힌트에 명확한 프로젝트 키워드를 추가합니다.
...
- **HWPFrame.HwpObject/pyhwpx 오류:** Windows·한글 설치, Python/한글 bitness, COM 보안 정책을 확인합니다. 우회하지 말고 TXT fallback을 사용합니다.
...
- `CONFIG_SCAFFOLDED`: config.local.json과 안전한 outbox/log가 준비됐지만 실제 수집 대상 repo 경로가 아직 없음
...
5. 원본 HWP 템플릿을 안전한 template 전용 경로에 둡니다.
